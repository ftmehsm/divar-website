...
function Posts() {
    const {data, isPending} = useQuery({queryKey:["allPosts"] , queryFn: getAllPosts });
    ...
    return (
        <div className="grid grid-flow-row-dense  grid-cols-1 sm:grid-cols-2 md:grid-cols-3 col-span-3 w-full  gap-3">
            {data?.data.posts.map(post => <div className="flex items-center gap-3 w-full max-h-[150px] p-4 cursor-pointer bg-white border rounded-md shadow-sm hover:bg-slate-100 transition-all" key={post._id}>
